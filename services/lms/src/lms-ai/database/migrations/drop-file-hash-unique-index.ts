...
export class DropFileHashUniqueIndexMigration {
  ...
  async run(): Promise<void> {
    try {
      ...
      if (fileHashUniqueIndex && fileHashUniqueIndex.name) {
        this.logger.log(
          `Found unique index on file_hash: ${fileHashUniqueIndex.name}`,
        );

        // Drop the unique index
        await collection.dropIndex(fileHashUniqueIndex.name);
        this.logger.log(
          `Successfully dropped unique index: ${fileHashUniqueIndex.name}`,
        );
        ...
        this.logger.log(
          'No unique index on file_hash found - migration not needed',
        );
      }
      ...
      if (!nonUniqueIndex) {
        ...
        this.logger.log(
          'Created non-unique index on file_hash for query performance',
        );
      }
    } catch (error) {
      this.logger.error(
