...
export class ReviewController {
  ...
  async createOrUpdateReview(@Payload() data: { itemType: any; itemId: string; userId: string; rating: number; title: string; body: string }) {
    const { itemType, itemId, userId, rating, title, body } = data;
    if (!itemType || !itemId || !userId || !rating) throw new Error('itemType, itemId, userId and rating are required');

    const review = await this.reviewService.createOrUpdate(itemType, itemId, userId, rating, title, body);
    return { message: 'Review created or updated successfully', data: review };
  }

  @MessagePattern({ cmd: 'library.reviews.getAll' })
  async listReviews(@Payload() data: { itemType: any; itemId: string; page?: number; limit?: number }) {
  ...
  async getMyReview(@Payload() data: { itemType: any; itemId: string; userId: string }) {
