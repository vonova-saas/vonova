...
import { BookProgress } from 'src/schemas/library/book/book-progress.schema';
...
@InjectModel(LibraryAsset.name) private readonly assetModel: Model<LibraryAsset>,
@InjectModel(BookProgress.name) private readonly bookProgressModel: Model<BookProgress>,
@InjectModel(Guide.name) private readonly guideModel: Model<Guide>,
@InjectModel(Presentation.name) private readonly presentationModel: Model<Presentation>,
...
this.bookModel.updateOne({ _id: book._id }, { $inc: { 'metrics.views': 1 } }).catch(() => {});


const progress = await this.bookProgressModel.findOne({ userId, bookId: book._id });
...
async updateBookProgress(bookId: string, userId: string, payload: { lastPage: number; timeSpentSec?: number; completed?: boolean }) {
const book = await this.bookModel.findById(bookId);
if (!book) throw new NotFoundException('Book not found');


const update = {
$set: { lastPage: payload.lastPage, completed: !!payload.completed, completedAt: payload.completed ? new Date() : undefined },
...
this.presentationModel.updateOne({ _id: pres._id }, { $inc: { 'metrics.views': 1 } }).catch(() => {});
return {
id: String(pres._id),
title: pres.title,
