...
import { Assignment } from '../../schemas/assignment/assignment.schema';
import { AssignmentAnswer } from '../../schemas/assignment/assignment-answer.schema';
import { CreateAssignmentDto, QuestionDto, UpdateAssignmentDto } from './dto/assignment.dto';

@Injectable()
export class AssignmentService {
  constructor(
    @InjectModel(Assignment.name) private assignmentModel: Model<Assignment>,
    @InjectModel(AssignmentAnswer.name) private answerModel: Model<AssignmentAnswer>,
  ) {}
  ...
  async updateAssignment(id: string, dto: UpdateAssignmentDto, userId?: string) {
    ...
    if (dto.noOfQuestions !== undefined) assignment.noOfQuestions = dto.noOfQuestions;

    if (dto.questions) {
      dto.questions.forEach((updatedQ: QuestionDto) => {
        const index = assignment.questions.findIndex(q => q.id === updatedQ.id);
        if (index !== -1) {
          assignment.questions[index] = { ...assignment.questions[index], ...updatedQ };
        } else {
  ...
  async submitAssignmentAnswers(
    ...
    let score = 0;
    const formatted: { questionId: string; selectedOptionId: string; correct: boolean }[] = [];
    ...
    for (const ans of answers) {
      ...
      if (correct) score++;
      formatted.push({ questionId: ans.questionId, selectedOptionId: ans.selectedOptionId, correct });
    }
  ...
  async getUserAttemptsForAssignment(assignmentId: string, userId?: string) {
    if (userId) {
      return this.answerModel.find({ assignment: assignmentId, userId }).sort({ createdAt: -1 });
    }
    return this.answerModel.find({ assignment: assignmentId }).sort({ createdAt: -1 });
  }
}
