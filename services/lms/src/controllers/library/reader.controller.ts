...
import { getMyFavorites, deleteFavorite, toggleFavorite } from "../../services/library/favorite.service";
...
export const addFavoriteController = asyncHandler(async (req: Request, res: Response) => {
  const data = await toggleFavorite(req.params.itemType as any, req.params.itemId, req.user!.id)
...
export const removeFavoriteController = asyncHandler(async (req: Request, res: Response) => {
  const data = await deleteFavorite(req.params.itemType as any, req.params.itemId, req.user!.id);
...
export const listMyFavoritesController = asyncHandler(async (req: Request, res: Response) => {
  const data = await getMyFavorites(req.user!.id, req.query.type as any);
