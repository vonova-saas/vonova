...
export class CourseController {
  ...
  updateCourse(@Payload() data: { courseId: string; dto: UpdateCourseDto; ownerId: string }) {
  ...
  publishCourse(@Payload() data: { courseId: string; dto: PublishCourseDto; ownerId: string }) {
    const { courseId, dto, ownerId } = data;
    if (!courseId || !dto || !ownerId) throw new Error('courseId, dto and ownerId are required');
  ...
  deleteCourse(@Payload() data: { courseId: string; ownerId: string }) {
    ...
    if (!courseId || !ownerId) throw new Error('courseId and ownerId are required');
