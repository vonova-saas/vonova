...
// Helper function to recompute rating aggregate
export const recomputeLibraryItemRatingAggregate = async (itemType: LibraryItemType, itemId: string) => {
  ...
  if (itemType === "BOOK") {
    await BookModel.updateOne({ _id: itemId }, update);
  }

  if (itemType === "GUIDE") {
    await GuideModel.updateOne({ _id: itemId }, update);
  }

  if (itemType === "PRESENTATION") {
    await PresentationModel.updateOne({ _id: itemId }, update);
  }
...
async function ensureItemExists(itemType: LibraryItemType, itemId: string) {
  let exists: any = null;
  if (itemType === "BOOK") {
    exists = await BookModel.findById(itemId);
  }

  if (itemType === "GUIDE") {
    exists = await GuideModel.findById(itemId);
  }

  if (itemType === "PRESENTATION") {
    exists = await PresentationModel.findById(itemId);
  }
