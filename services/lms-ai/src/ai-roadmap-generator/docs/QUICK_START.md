...
This guide will help you quickly set up and test the communication between your AI service (port 5000) and backend service (port 4004).
...
#### Terminal 2: Start Backend Service (Port 4004)
```bash
cd services/lms-ai
bun run dev
```
...
|---------|------|-----|---------|
| **Backend** | 4004 | `http://localhost:4004` | Main API service |
...
curl http://localhost:4004/api/roadmap/health

# Communication Test
curl http://localhost:4004/api/roadmap/test-ai-connection
```

### Generate Test Roadmap
```bash
curl -X POST http://localhost:4004/api/roadmap/generate \
...
#### ❌ "Port already in use"
**Problem**: Port 4004 or 5000 is occupied
**Solution**:
```bash
...
**Solution**: Check the API documentation at `http://localhost:4004/api-docs`
...
1. **Frontend Integration**: Connect your frontend to `http://localhost:4004/api`
