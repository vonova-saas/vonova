...
export class ReaderGatewayController {
  ...
  @ApiOperation({
    ...
    description: 'Retrieves the content of a specific book for the authenticated user.',
  })
  @ApiParam({
    name: 'bookId',
  ...
  @HttpCode(200)
  async updateBookProgress(@Param('bookId') bookId: string, @Body() dto: UpdateReaderBookProgressDto, @Request() req: any) {
    const userId = req.user?.id || req.user?.sub;
