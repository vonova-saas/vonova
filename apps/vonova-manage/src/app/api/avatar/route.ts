...
/** Keys we allow fetching via SDK (same bucket as admin profile uploads). */
function isAllowedS3ObjectKey(key: string): boolean {
  return key.startsWith("avatars/") || key.startsWith("admins/");
}
...
async function fetchObjectViaS3Sdk(
  ...
  if (!isAllowedS3ObjectKey(key)) return null;
