...
const CV_KEY_PREFIX = "instructor-cvs/";
const MAX_CV_BYTES = 50 * 1024 * 1024; // 50MB max for CVs
...
async function fetchObjectViaS3Sdk(
  ...
  if (!key.startsWith(CV_KEY_PREFIX)) return null;
  ...
  try {
    ...
    const contentType =
      out.ContentType?.split(";")[0]?.trim() || "application/pdf";
