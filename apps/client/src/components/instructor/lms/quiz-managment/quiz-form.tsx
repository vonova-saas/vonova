...
export default function InstructorQuizForm({ quiz, onCancel, onCreate, onUpdate }: Props) {
  ...
  const canSave = useMemo(() => {
    const noQs = typeof noOfQuestions === "string" ? noOfQuestions : String(noOfQuestions ?? "");
    if (!title.trim() || !topic.trim() || !noQs.trim()) return false;
  ...
  const handleSubmit = async () => {
    try {
      ...
      const payload: Omit<QuizType, "_id"> = {
        ...
        topic: topic.trim(),
        noOfQuestions: typeof noOfQuestions === "string" ? noOfQuestions : String(noOfQuestions ?? ""),
        questions,
      };
      if (isEdit && quiz) {
