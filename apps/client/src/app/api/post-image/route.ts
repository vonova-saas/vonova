...
function isAllowedS3Host(hostname: string): boolean {
...
function s3CommunityEnv() {
  const region = process.env.AWS_S3_REGION_APP_COMM?.trim();
  const accessKeyId = process.env.AWS_S3_ACCESS_KEY_ID_APP_COMM?.trim();
  const secretAccessKey = process.env.AWS_S3_SECRET_ACCESS_KEY_APP_COMM?.trim();
  const bucket = process.env.AWS_S3_BUCKET_APP_COMM?.trim();
...
async function fetchObjectViaS3Sdk(
  ...
  const env = s3CommunityEnv();
  if (!env || bucket !== env.bucket) return null;
  if (!key.startsWith(POST_KEY_PREFIX)) return null;
...
export async function GET(req: NextRequest) {
  ...
  if (!isAllowedS3Host(target.hostname)) {
